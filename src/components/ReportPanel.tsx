...
import { Download, FileText, Settings, ShieldAlert, Cpu, Zap, Battery, LineChart, Hash, Server, DollarSign } from 'lucide-react';
...
export function ReportPanel({ state, results }: { state: AppState, results: CalculationResults }) {
  ...
  const GridItem = ({ label, value, unit, icon: Icon, warn = false }: any) => (
    <div className={`p-4 border-b border-line bg-white ${warn ? 'border-l-4 border-l-[#D35400]' : ''}`}>
  ...
  return (
    <div className="flex flex-col h-full bg-[#F9F9F7] print:bg-white print:w-full print:absolute print:inset-0 print:p-8">
      ...
      <div className="flex justify-between items-center p-6 border-b border-line print:hidden">
        ...
        <div className="flex gap-2">
          <button onClick={handlePrint} className="p-2 border border-line bg-ink text-white hover:bg-[#333] rounded cursor-pointer flex items-center gap-2 transition-colors">
            <FileText className="w-4 h-4" />
            <span className="text-xs uppercase font-bold tracking-wider">Gerar PDF</span>
          </button>
        </div>
      </div>
      ...
      <div className="p-6 space-y-6 flex-1 overflow-y-auto print:p-0 print:overflow-visible">
        ...
        <div className="print:break-inside-avoid">
           ...
           {results.vocMaxTemp > (state.equipment.inverterMaxDcV || Infinity) && (
              <div className="bg-[#FFF9F5] border border-[#D35400] text-[#D35400] text-[10px] p-2 mt-2 font-mono">
        ...
        <div className="print:break-before-page mt-12 mb-12">
          <h3 className="text-[10px] bg-ink text-white inline-block px-2 py-1 font-mono uppercase mb-4">5. Memória de Cálculo (Engenharia)</h3>
          <div className="space-y-4">
            {results.calculationMemory.map((mem, idx) => (
              <div key={idx} className="bg-white border border-line p-4">
        ...
        <div className="print:break-before-page mt-12 pt-8 print:mt-0 print:pt-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-px bg-line border border-line mb-6">
             <div className="p-6 bg-white border-b border-r border-line text-center md:col-span-2">
          ...
          <ul className="text-sm font-mono space-y-2 border border-line p-5 bg-white text-ink leading-relaxed">
