...
import { Sun, CheckSquare, Folder, Cloud, Zap, Database, Cpu, ShieldAlert, DollarSign, PieChart as ChartIcon } from 'lucide-react';

const defaultState: AppState = {
  project: {
    clientName: 'Demo Ltda', city: 'Sao Paulo', state: 'SP', lat: -23.5505, lng: -46.6333, altitude: 760, utility: 'Enel', tariffGroup: 'B3', gridPhase: 'Trifásico', gridVoltage: 220, contractedDemand: 30
  },
  climate: {
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-bg text-ink font-sans flex flex-col h-screen overflow-hidden print:h-auto print:overflow-visible">
      
      {/* GLOBAL HEADER (Hidden in print) */}
      <header className="h-16 bg-white border-b border-line flex items-center justify-between px-8 flex-shrink-0 print:hidden z-10 shadow-sm relative">
        <div className="flex items-center gap-3">
          <Sun className="w-5 h-5 text-accent" />
          <h1 className="text-lg font-serif italic m-0 text-ink tracking-wide">
            PvStudio Pro <span className="font-sans font-bold text-xs uppercase bg-black text-white px-2 py-1 not-italic ml-2 tracking-widest rounded-sm">Enterprise</span>
          </h1>
        </div>
        <div className="flex gap-4 font-mono text-xs items-center">
          ...
          <div className="flex items-center gap-1 text-[#27AE60] bg-[#E8F8F5] px-2 py-1 rounded">
      ...
      <main className="flex-1 overflow-hidden flex print:flex-col relative">
        ...
        {/* SIDEBAR NAV */}
        <aside className="w-64 bg-[#EEECE6] border-r border-line flex flex-col print:hidden">
          ...
          <nav className="flex-1 overflow-y-auto py-4">
            {MODULES.map(mod => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-6 py-3 text-xs uppercase tracking-wider font-bold transition-colors border-l-4 ${activeTab === mod.id ? 'bg-black/5 border-accent text-ink' : 'border-transparent text-[#666] hover:text-ink hover:bg-black/5'}`}
              >
